...
jQuery(function () {
    ...
    jQuery.validator.addMethod("pattern", function (value, element) {
        ...
        return result;
    }, jQuery.validator.format("输入的内容不符合要求"));

    /**
     * Validate Ajax验证表单
     * @author zoujingli <zoujingli@qq.com>
     */
    jQuery.validator.addMethod('data-ajaxcheck', function (value, element) {
        if (this.optional(element)) {
            return true;
        }
        var url = $(element).attr('data-ajaxcheck');
        if (value.indexOf('http') !== false) {
            var result = false;
            $.ajax({
                url: url,
                data: {value: value, name: $(element).attr('name')},
                async: false,
                error: function () {
                    result = false;
                },
    ...
    }, jQuery.validator.format("输入的内容未能通过检测"));

    /**
     * 内容再次验证
     */
    jQuery.validator.addMethod('data-checktarget', function (value, element) {
        var name = $(element).attr('data-checktarget');
        var oldvalue = $('[name="' + name + '"]').val();
        if (oldvalue !== value) {
            return false;
        }
        return true;
